...
def get_cds_data():
	cds_data = pd.DataFrame(columns=['maturity', 'spread', 'rate'],
		data = [[0.5, -0.0028, 0.0063], [1.0, -0.0024, 0.0073], [2.0, -0.0017, 0.0091], 
				[3.0, -0.008, 0.0110], [4.0, 0.002, 0.0136], [5.0, 0.0014, 0.016]])
	return cds_data
...
def at1p(V0, H0, B, sigma, r, t):
	...
	for i in range(1,len(t)):
		Ht[i] = (H0/V0) * V[t] * np.exp(-B*sigma2u[i])
	...
	result = pd.DataFrame(data=[t,Vt,Ht,survival_prob, intensity])
	result.columns = ['Maturity', 'Firm Value', 'H', 'Survivial Probability', 'Default Intensity']
	return result


cds_data = get_cds_data()
at1p(V0=1, H0=0.7,B=0.4,sigma=np.repeat(0.1, 10), r=cds_data.rate, t=cds_data.maturity)
