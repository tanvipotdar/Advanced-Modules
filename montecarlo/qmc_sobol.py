...
# European call option using Sobol points
#
...
for it in range(1,4):
    ...
    plt.legend((' Error',' MC error bound'), loc=2, fontsize=14)
    plt.axis([1, 4096, 0.005, 10])
    if   it==1:
        plt.title('comparison to exact solution')
        plt.savefig('lec13a.eps', format='eps', dpi=1200)
    elif it==2:
        plt.title('comparison to exact solution')
        plt.savefig('lec13b.eps', format='eps', dpi=1200)
    else:
        plt.title('comparison to exact solution')
        plt.savefig('lec13c.eps', format='eps', dpi=1200)
